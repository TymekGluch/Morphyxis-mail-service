...
- `ghcr.io/morphyxis/morphyxis-mail-service` - contains docker images for morphyxis mail service
  - `1.1.3` - latest stable version of the image (tagged with git tag & build manually triggered)
  - `beta` - latest beta version of the image (built on pull request)

to pull the image, execute command

```bash
  docker pull ghcr.io/morphyxis/morphyxis-mail-service:1.1.3
```
...
docker run -d \
  ...
  -p 8080:8080 \
  ghcr.io/morphyxis/morphyxis-mail-service:1.1.3
```
